...
/**
  ...
  *     Update for new scheduling algorithms and assignment 4
  *
  * @version 1.04
  *     Benjamin Estela (8 May 2019)
  *     Update for new scheduling algorithms and assignment 5
  *
...
//
    ...
    { //make; ./sim5 config_1.conf
        ...
        //srand(time(NULL));
        ReadMeta( log, metaFile, config, pcb, T1 );
        metaFile.close();
        log.close();
        return 0;
    }
  ...
  */
    void logConfig( Config config, std::ofstream& log ){
        if( config.getLogLocation() == "Log to Monitor" || config.getLogLocation() == "Log to Both")
        {
            ...
            cout << "Processor Quantum Number = " << config.getQuantum() << " ms/cycle" << endl;
            cout << "CPU Scheduling Code = " << config.getSchedule() << endl;
        ...
        if( config.getLogLocation() == "Log to File" || config.getLogLocation() == "Log to Both")
        {
            ...
            log << "Processor Quantum Number = " << config.getQuantum() << endl;
            log << "CPU Scheduling Code = " << config.getSchedule() << endl;
